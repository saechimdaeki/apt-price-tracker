...
네이버 부동산 아파트 매물을 매일 수집해 **같은 단지·같은 평형 매물의 평균 호가와 비교**하고, GitHub Pages 웹 리포트와 Teams 알림으로 공유하는 배치 프로젝트입니다.
...
평균 호가는 **같은 단지 + 같은 전용면적(㎡ 반올림)** 매물끼리만 냅니다. 한 단지에 20평대와 50평대가 섞여 있어도 서로 비교하지 않습니다. 같은 평형 매물이 한 건뿐이면 비교 기준이 없으므로 `비교 대상 없음`으로 표시하고 급매/저렴 판정에서 제외합니다.
...
카드는 웹 리포트와 같은 기준(수집 시각, 같은 평형 평균 대비 급매/저렴 정의)으로 맞춰져 있고, 가장 싸게 나온 매물 3건을 단지 중복 없이 보여줍니다. 각 행을 누르면 해당 매물로, 버튼을 누르면 웹 리포트로 이동합니다.
...
매물 행에는 비교에 쓰인 표본 정보도 함께 담깁니다 — `peerCount`(같은 평형 매물 수), `peerAreaText`(예: `전용 85㎡`), `avgPrice`/`avgPriceText`(같은 평형 평균 호가).
